...
for f in "$WAV_DIR"/*.wav; do
  ...
  dotnet run -- \
    ...
    --audio "$f" \
    --out "$out_json" \
    --vad-onset 0.95 --vad-offset 0.75 --vad-min-silence 0.6 --vad-min-speech 0.2 \
    --vad-pad-onset 0.0 --vad-pad-offset 0.0 --vad-smoothing none --vad-overlap 0.5 \
    --cluster-search 0 --merge-gap 0.25 --min-seg 1.0 \
    --subseg-win 4.0 --subseg-shift 4.0

  # count segments & speakers
  segs=$(python3 - <<PY
import json
